...
if(isset($_POST['updateflight']))
{
    ...
    $leavingfrom = trim($_POST['leavingfrom']);
    $goingto = trim($_POST['goingto']);
    $departing = trim($_POST['departing']);
    $returning = trim($_POST['returning']);
    $adults = trim($_POST['adults']);
    $children = trim($_POST['children']);
    $flighttype = trim($_POST['flighttype']);
    $airline = trim($_POST['airline']);
    $travelclass = trim($_POST['travelclass']);
    $amount = trim($_POST['amount']);

    $updateflight = "UPDATE roundtripflights set Name = '$name', Email = '$email', TripType = '$triptype', LeavingFrom = '$leavingfrom', GoingTo = '$goingto', Departing = '$departing', Returning = '$returning', Adults = '$adults', Children = '$children', FlightType = '$flighttype', Airline = '$airline', TravelClass = '$travelclass', Amount = '$amount' where Id = $id";

    $execute = mysqli_query($connect, $updateflight);

    if(isset($execute))
    {
        echo "<script type='text/javascript'>alert('Flight has been updated');</script>";
        header("Location: RoundtripRecordsA.php");
    }
    else
    {
        echo "<script type='text/javascript'>alert('Flight has not been updated');</script>";
    }
}
                            ...
                            <div class="card-body p-0">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-lg-7">
                                        <div class="p-5">
                                            <div class="">
                                                <h4 class="" style="color:#4e73df; font-family:Heebo"><strong>Edit a Roundtrip flight</strong> <i class="fa fa-edit"></i></h4><br />
                                            </div>
                                                        ...
                                                        <form method="post" class="user">
                                                            ...
                                                            <!--Book-->
                                                            <div>
                                                                <input name="updateflight" type="submit" value="SAVE CHANGES" class="btn btn-primary btn-block" style="font-size:13px; font-weight:bold" />
                                                            </div>
